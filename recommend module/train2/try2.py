...
film_ids = [117895, 119145, 120466, 122882, 122900, 122902, 122906, 122912, 122916, 122918, 122920, 122924, 122922, 130634, 131739, 133195, 135288, 136556, 136864, 135536, 113278, 111659, 146309, 3997, 88448, 58826, 110102, 37731, 55805, 90345]
...
# Фильтруйте рекомендации по году выпуска, жанру и исключите уже оцениванные фильмы
target_genres = ['Comedy']  # Укажите целевые жанры
start_year = 2010
end_year = 2020
...
for film_index in recommended_films:
    ...
    if film_id in id_to_year and id_to_year[film_id] is not None and start_year <= id_to_year[film_id] <= end_year:
        ...
        if any(genre in target_genres for genre in genres) and film_id not in film_ids:
